...
#if defined(NDEBUG)
# error "LGBTCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x813677a57b93af547c27611b4fe8fb972b85d5a6c3c3e65dfe229f76f2ccb64a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LGBTCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LGBTCoin Signed Message:\n";
...
{
    // LGBTCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LGBTCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LGBTCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1508544617, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The coin is created to support LGBT community";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0444b24a67d48f208795e3f22d14b35975f65220ebbf3d23e102df0b759102c5919a130f2ae69d07e5dfb6a2e7c1e70b96ef987b23f8a0f97a73b887abbc037a69") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508544617;
        ...
        assert(block.hashMerkleRoot == uint256("0x2decea17416a8f5fb6abdf09a3e87fe29ba09d5a8a75ace85eba15d10a2b42f5"));
...
unsigned char pchMessageStart[4] = { 0xda, 0xe2, 0x9b, 0x1a }; // LGBTCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LGBTCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LGBTCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LGBTCoinMiner(CWallet *pwallet)
{
    printf("LGBTCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lgbtcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LGBTCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LGBTCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LGBTCoinMiner, pwallet));
}
